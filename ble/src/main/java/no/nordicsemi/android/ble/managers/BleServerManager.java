...
import no.nordicsemi.android.converted.annotation.CharacteristicPermissions;
import no.nordicsemi.android.converted.annotation.CharacteristicProperties;
import no.nordicsemi.android.converted.annotation.DescriptorPermissions;
import no.nordicsemi.android.converted.data.Data;
import no.nordicsemi.android.converted.observer.ServerObserver;
import no.nordicsemi.android.converted.utils.ILogger;

/**
 * The manager for local GATT server. To be used with one or more instances of {@link BleClientManager}
 *
 * @since 2.2
 */
...
public abstract class BleServerManager implements ILogger {
	...
	private final List<BleClientManager> managers = new ArrayList<>();
	...
	public final void close() {
		...
		serverServices = null;
		for (BleClientManager manager: managers) {
	 ...
	 */
	final void addManager(@NonNull final BleClientManager manager) {
	 ...
	 */
	final void removeManager(@NonNull final BleClientManager manager) {
		managers.remove(manager);
	}

	public final boolean isShared(@NonNull final BluetoothGattCharacteristic characteristic) {
		return sharedCharacteristics != null && sharedCharacteristics.contains(characteristic);
	}

	public final boolean isShared(@NonNull final BluetoothGattDescriptor descriptor) {
	...
	private BleManagerHandler getRequestHandler(@NonNull final BluetoothDevice device) {
		for (final BleClientManager manager : managers) {
