package no.nordicsemi.android.ble.Handlers;
...
import WriteRequest;
import no.nordicsemi.android.converted.utils.Bytes;
import no.nordicsemi.android.ble.aaaaaaa.BleClientManager;
import no.nordicsemi.android.ble.managers.BleServerManager;
...
import no.nordicsemi.android.converted.annotation.PhyValue;
...
public abstract class BleManagerHandler extends RequestHandler {
	 ...
	 * returns true and the device got disconnected without calling {@link BleClientManager#disconnect()}
	 ...
	 * {@link BleClientManager#connect(BluetoothDevice)} again. This time this method will call
	...
	/**
	 * The connect request. This is instantiated in {@link BleClientManager#connect(BluetoothDevice)}
	 ...
	 */
	@Nullable
	private TimeoutableRequest.AwaitingRequest awaitingRequest;
	 ...
	 */
	public void init(@NonNull final BleClientManager manager, @NonNull final Handler handler) {
	 ...
	 */
	public void useServer(@Nullable final BleServerManager server) {
	...
	private boolean internalConnect(@NonNull final BluetoothDevice device,
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
			...
			bluetoothGatt = device.connectGatt(context, false, gattCallback, BluetoothDevice.TRANSPORT_LE, preferredPhy/*, handler*/);
	...
	private boolean ensureServiceChangedEnabled() {
		...
		final BluetoothGattService gaService = gatt.getService(BleClientManager.GENERIC_ATTRIBUTE_SERVICE);
		if (gaService == null)
			return false;

		final BluetoothGattCharacteristic scCharacteristic =
				gaService.getCharacteristic(BleClientManager.SERVICE_CHANGED_CHARACTERISTIC);
	...
	private boolean internalEnableNotifications(@Nullable final BluetoothGattCharacteristic characteristic) {
		...
		if (descriptor != null) {
			...
			log(Log.DEBUG, "gatt.writeDescriptor(" +
					BleClientManager.CLIENT_CHARACTERISTIC_CONFIG_DESCRIPTOR_UUID + ", value=0x01-00)");
	...
	private boolean internalDisableNotifications(@Nullable final BluetoothGattCharacteristic characteristic) {
		...
		if (descriptor != null) {
			...
			log(Log.DEBUG, "gatt.writeDescriptor(" +
					BleClientManager.CLIENT_CHARACTERISTIC_CONFIG_DESCRIPTOR_UUID + ", value=0x00-00)");
			return internalWriteDescriptorWorkaround(descriptor);
		}
		return false;
	}
	...
	private boolean internalEnableIndications(@Nullable final BluetoothGattCharacteristic characteristic) {
		...
		if (descriptor != null) {
			...
			log(Log.DEBUG, "gatt.writeDescriptor(" +
					BleClientManager.CLIENT_CHARACTERISTIC_CONFIG_DESCRIPTOR_UUID + ", value=0x02-00)");
			return internalWriteDescriptorWorkaround(descriptor);
		}
		return false;
	}
	...
	private boolean internalSendNotification(@Nullable final BluetoothGattCharacteristic serverCharacteristic,
		...
		if ((serverCharacteristic.getProperties() & requiredProperty) == 0)
			return false;
		final BluetoothGattDescriptor cccd = serverCharacteristic.getDescriptor(BleClientManager.CLIENT_CHARACTERISTIC_CONFIG_DESCRIPTOR_UUID);
		if (cccd == null)
			return false;
	...
	private static BluetoothGattDescriptor getCccd(@Nullable final BluetoothGattCharacteristic characteristic,
		...
		return characteristic.getDescriptor(BleClientManager.CLIENT_CHARACTERISTIC_CONFIG_DESCRIPTOR_UUID);
	}
	 ...
	 */
	@NonNull
	public ValueChangedCallback getValueChangedCallback(@Nullable final Object attribute) {
	 ...
	 */
	public void removeValueChangedCallback(@Nullable final Object attribute) {
	...
	@Override
	public final void enqueue(@NonNull final Request request) {
		final Deque<Request> queue = initInProgress ? initQueue : taskQueue;
		queue.add(request);
		request.enqueued = true;
		nextRequest(false);
	...
	@Override
	public final void onRequestTimeout(@NonNull final TimeoutableRequest request) {
		this.request = null;
		awaitingRequest = null;
	...
	public final void overrideMtu(@IntRange(from = 23, to = 517) final int mtu) {
	 ...
	 * {@link BleClientManager#useServer(BleServerManager)} and opened using {@link BleServerManager#open()}.
	 *
	 * @param server The GATT Server instance. Use {@link BluetoothGattServer#getService(UUID)} to
	 *               obtain service instance.
	 */
	public void onServerReady(@NonNull final BluetoothGattServer server) {
		...
		public void onCharacteristicChanged(final BluetoothGatt gatt,
			...
			if (isServiceChangedCharacteristic(characteristic)) {
				...
				final BluetoothGattDescriptor cccd =
						characteristic.getDescriptor(BleClientManager.CLIENT_CHARACTERISTIC_CONFIG_DESCRIPTOR_UUID);
	...
	public final void onCharacteristicReadRequest(@NonNull final BluetoothGattServer server,
	...
	public final void onCharacteristicWriteRequest(@NonNull final BluetoothGattServer server,
	...
	public final void onDescriptorReadRequest(@NonNull final BluetoothGattServer server,
	...
	public final void onDescriptorWriteRequest(@NonNull final BluetoothGattServer server,
											   @NonNull final BluetoothDevice device, final int requestId,
											   @NonNull final BluetoothGattDescriptor descriptor,
											   final boolean preparedWrite, final boolean responseNeeded,
											   final int offset, @NonNull final byte[] value) {
	...
	public final void onExecuteWrite(@NonNull final BluetoothGattServer server,
									 @NonNull final BluetoothDevice device, final int requestId,
	...
	public final void onNotificationSent(@NonNull final BluetoothGattServer server,
	...
	public final void onMtuChanged(@NonNull final BluetoothGattServer server,
								   @NonNull final BluetoothDevice device,
								   final int mtu) {
	...
	private synchronized void nextRequest(final boolean force) {
		...
		if (request instanceof TimeoutableRequest.AwaitingRequest) {

			final TimeoutableRequest.AwaitingRequest r = (TimeoutableRequest.AwaitingRequest) request;
	...
	private boolean isServiceChangedCCCD(@Nullable final BluetoothGattDescriptor descriptor) {
		return descriptor != null &&
				BleClientManager.SERVICE_CHANGED_CHARACTERISTIC.equals(descriptor.getCharacteristic().getUuid());
	}
	...
	private boolean isServiceChangedCharacteristic(@Nullable final BluetoothGattCharacteristic characteristic) {
		return characteristic != null &&
				BleClientManager.SERVICE_CHANGED_CHARACTERISTIC.equals(characteristic.getUuid());
	}
	...
	private boolean isCCCD(@Nullable final BluetoothGattDescriptor descriptor) {
		return descriptor != null &&
				BleClientManager.CLIENT_CHARACTERISTIC_CONFIG_DESCRIPTOR_UUID.equals(descriptor.getUuid());
	}
